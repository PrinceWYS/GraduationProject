...
class Location:
    ...
    def CheckImageAndLocate(self, image_path):
        if image_path is None:
            ...
            if not self.m_ImageInit:
                ...
                # if not self.m_ARFoundation.TryGetImageDimensions(ref m_ImageWidth, ref m_ImageHeight):
            ...
            # if not self.m_ARFoundation.TryGetLatestImage(m_ImageBufferOrig):
            #    InvokeResultCallback(ARLocationStatus.CHECK_IMAGE_GET_FAIL)
            #    return
            # preProcStereoImage()
    ...
    def OnLocationResultReturn(self, result, isResponse):
        ...
        file_write_obj = open(self.myPath, 'a')
        self.tum = result['requestTimestamp'] + " " + str(pose[3]) + " " + str(pose[7]) + " " + str(
            pose[11]) + " " + str(qx) + " " + str(qy) + " " + str(qz) + " " + str(qw)
        print(self.tum)
        file_write_obj.writelines(self.tum+'\n')
    ...
    def RequestLocateNewApi(self, requestData, callback):
        requestUrl = \
            'https://mr-stage.sensetime.com/api/app/positioning/scheduler/v1/regions/gAj84noD?'\
        ...
        headers = {
            ...
            "App-Key": '7136ac79f678473abdeeeec8fb32b859',
            # "Request-Timestamp": self.GetTimeStamp1970(),
            "Request-Timestamp": self.picTime,
            "Request-Uuid": uid,
    ...
    class Config:
        def __init__(self):
            self.appKey = '7136ac79f678473abdeeeec8fb32b859'
            self.appSecret = '8016d055b14244fb925f'
            self.Location_Base_URL = 'https://mr-stage.sensetime.com/api/app/positioning/scheduler/v1/regions/gAj84noD?'\
                                     'appKey=7136ac79f678473abdeeeec8fb32b859&appSecret=8016d055b14244fb925f&algoVersion=v4'\
                                     '&buildingCode=Xr3GMF8Y&blockCode=wtb2IUh3&regionCode=gAj84noD&tenantId=a93afefb-62e7-4c5a-90ab-f3768a23434a'
    ...
    def capture(self):
        ...
        try:
            for i in range(500):
                ...
                t = self.GetTimeStamp1970()
                timeList.append(t)
                cv2.imwrite('./images/'+t+'.png', color_image)
...
if __name__ == '__main__':
    if os.path.exists("./camera.tum"):
        os.remove("./camera.tum")
        print("remove")
    # global s
    # global data
	# s.send(json.dumps(data).encode('utf-8'))
    while True:
        timeList = Camera().capture()
        for t in timeList:
            Location().RequestOnceLocation('/home/cvg/Downloads/location/images/'+t+'.png')
        # Location().RequestOnceLocation('/home/cvg/Downloads/location/Meng.JPG')
        break